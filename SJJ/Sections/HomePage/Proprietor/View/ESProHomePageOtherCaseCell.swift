//
//  ESProHomePageOtherCaseCell.swift
//  Consumer
//
...
protocol ESProHomePageOtherCaseCellDelegate: NSObjectProtocol {
    func getOtherData(_ index: Int) -> ESProHomePageCommon?
}

class ESProHomePageOtherCaseCell: UICollectionViewCell, ESProHomePageCellCommonProtocol {
    ...
    func updateCell(_ indexPath: IndexPath) {
        ...
        if let model = delegate?.getOtherData(indexPath.item) {
            ...
            styleLabel.text = model.extend_dic?.esportTag
    ...
    private func setUpView() {
        ...
        styleLabel.snp.makeConstraints { (make) in
            make.left.equalTo(designerName)
            make.top.equalTo(designerName.snp.bottom).offset(1.scalValue)
            make.height.equalTo(17.scalValue)
            make.right.equalTo(likeImgView.snp.left).offset(-10.scalValue).priority(800)
        }
        styleLabel.setContentHuggingPriority(UILayoutPriority(249), for: .horizontal)
    ...
    private lazy var imgTagView: UIView = {
        let view = UIView()
        view.layer.cornerRadius = 2.0
        view.backgroundColor = ESColor.color(hexColor: 0x6A91FE, alpha: 1.0)
