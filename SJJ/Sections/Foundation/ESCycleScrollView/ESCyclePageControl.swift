...
private let ESPageControlPointWidth: CGFloat = 7
private let ESPageControlPointHeight: CGFloat = 20

enum ESCyclePageControlAliment {
    case CenterBottom
    case RightBottom
    case LeftBottom
}

class ESCyclePageControl: UIPageControl {
    ...
    var pageSize: CGSize {
        get {
            if let curImage = currentImage, let defImage = defaultImage {
                ...
                let pageW = CGFloat(numberOfPages - 1) * (pointSpace + ESPageControlPointWidth) + ESPageControlPointWidth
                return CGSize(width: pageW, height: ESPageControlPointHeight)
            }
        }
    }
...
extension ESCyclePageControl {
    ...
    func getDotSize(currentIndex:Int) -> CGSize {
        ...
        if let curImage = currentImage, let defImage = defaultImage {
            ...
            newSize = CGSize(width: ESPageControlPointWidth, height: ESPageControlPointWidth)
        }
        return newSize
    }
}
