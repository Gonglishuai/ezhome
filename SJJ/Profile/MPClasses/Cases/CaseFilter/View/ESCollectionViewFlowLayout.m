...
NSString *const ESCollectionViewSectionBackground = @"ESCollectionViewSectionBackground";

@implementation ESCollectionViewFlowLayout
...
- (void)setup {
    [self registerClass:[ESCollectionReusableView class] forDecorationViewOfKind:ESCollectionViewSectionBackground];
}
...
- (void)prepareLayout {
    ...
    if (!numberOfSections || ![delegate conformsToProtocol:@protocol(ESCollectionViewFlowLayoutDelegate)]) {
        return;
    }
    ...
    for (NSInteger section = 0; section < numberOfSections; section++) {
        ...
        // 2、定义
        ESCollectionViewLayoutAttributes *attr = [ESCollectionViewLayoutAttributes layoutAttributesForDecorationViewOfKind:ESCollectionViewSectionBackground withIndexPath:[NSIndexPath indexPathForItem:0 inSection:section]];
...
{
    if ([elementKind isEqualToString:ESCollectionViewSectionBackground]) {
