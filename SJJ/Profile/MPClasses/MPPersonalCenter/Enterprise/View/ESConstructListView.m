...
@interface ESConstructListView ()<UITableViewDelegate, UITableViewDataSource>

@end

@implementation ESConstructListView
{
...
- (void)initUI
{
    ...
    [_tableView registerNib:[UINib nibWithNibName:@"ESConstructListCell" bundle:nil]
     forCellReuseIdentifier:@"ESConstructListCell"];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    NSInteger rows = 0;
    if (self.viewDelegate
        && [self.viewDelegate respondsToSelector:@selector(getConstructListRowsWithSection:)])
    {
        rows = [self.viewDelegate getConstructListRowsWithSection:section];
    }
    return rows;
}
...
{
    static NSString *cellID = @"ESConstructListCell";
    ESConstructListCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID
