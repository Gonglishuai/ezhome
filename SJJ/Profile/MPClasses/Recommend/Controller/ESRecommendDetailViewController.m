//
//  ESRecommendDetailViewController.m
//  Consumer
//
...
@interface ESRecommendDetailViewController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation ESRecommendDetailViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    
    self.titleLabel.text = @"商品推荐";
...
- (void)setTableView {
    ...
    [_tableView registerNib:[UINib nibWithNibName:@"ESRecommentProductTableViewCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:@"ESRecommentProductTableViewCell"];
...
- (void)getData {
    ...
    [ESRecommendAPI getRecommendDetailWithRecommendId:_myRecommendId Success:^(NSDictionary *dict) {
        ...
        NSArray *list = dict[@"productList"] ? dict[@"productList"] : [NSArray array];
        ...
        if (weakSelf.datasSourse.count == 0) {
            [weakSelf showNoDataIn:weakSelf.tableView imgName:@"nodata_datas" frame:weakSelf.tableView.bounds Title:@"啊哦~暂时没有数据~" buttonTitle:nil Block:nil];
        } else {
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (1 == section) {
        ...
        NSString *num = [NSString stringWithFormat:@"共%@件商品", _datasDic[@"itemCount"]?_datasDic[@"itemCount"]:@""];
        [header setTitle:@"推荐清单" titleColor:[UIColor stec_titleTextColor] subTitle:num subTitleColor:[UIColor stec_subTitleTextColor] backColor:[UIColor stec_viewBackgroundColor]];
        return header;
    } else {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        ...
        [cell setInfo:_datasDic];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        return cell;
    } else {
        ESRecommentProductTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:@"ESRecommentProductTableViewCell" forIndexPath:indexPath];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section-1 < _datasSourse.count) {
        ...
        NSString *goodid = [NSString stringWithFormat:@"%@", info[@"spu"] ? info[@"spu"] : @""];
