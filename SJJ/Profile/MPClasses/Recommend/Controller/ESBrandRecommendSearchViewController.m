//
//  ESBrandRecommendSearchViewController.m
//  Consumer
//
...
#define Brand_Recommend_SEARCH_HISTORY @"brandRecommendSearchHistory"

@interface ESBrandRecommendSearchViewController ()<UITextFieldDelegate, UITableViewDataSource,UITableViewDelegate>
...
@implementation ESBrandRecommendSearchViewController
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refresh) name:Brand_Recommend_Change object:nil];
...
- (void)loadSearchHistory {
    ...
    NSMutableArray *searchRecords = (NSMutableArray *)[defaults objectForKey:Brand_Recommend_SEARCH_HISTORY];
...
- (void)addHistory:(NSString *)searchStr {
    if ([self.historyDatasSource indexOfObject:searchStr] == NSNotFound) {
        ...
        NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
        [defaults setObject:self.historyDatasSource forKey:Brand_Recommend_SEARCH_HISTORY];
        [self.historyTableView reloadData];
    } else {
...
- (void)clearHistory {
    [self.historyDatasSource removeAllObjects];
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    [defaults removeObjectForKey:Brand_Recommend_SEARCH_HISTORY];
    [self.historyTableView reloadData];
...
- (void)searchRequest {
    ...
    [ESRecommendAPI getBrandRecommendListWithName:[_searchString stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding] pageSize:_pageSize pageNum:_pageNum Success:^(NSDictionary *dict) {
