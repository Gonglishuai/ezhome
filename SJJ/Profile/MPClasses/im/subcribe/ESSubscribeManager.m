...
NSString *const ESSubscribeNetState = @"net_state";

NSString *const ESSubscribeOnlineState = @"online_state";

@interface ESSubscribeManager()<NIMEventSubscribeManagerDelegate,NIMConversationManagerDelegate,NIMLoginManagerDelegate,NIMUserManagerDelegate>
...
@implementation ESSubscribeManager

+ (instancetype)sharedManager {
    return nil;
//    static ESSubscribeManager *instance;
//    static dispatch_once_t onceToken;
//    dispatch_once(&onceToken, ^{
//        instance = [[ESSubscribeManager alloc] init];
//    });
//    return instance;
}
...
- (void)start {
    SHLog(@"SubscribeManager Center Setup");
}
...
- (void)subscribeTempUserOnlineState:(NSString *)userId {
    ...
    if (isRobot || isMe) {
        SHLog(@"user can not subscribe temp publisher: %@",userId);
        //自己或者机器人，则不需要订阅
        return;
    }
    ...
    [[NIMSDK sharedSDK].subscribeManager subscribeEvent:request completion:^(NSError * _Nullable error, NSArray * _Nullable failedPublishers) {
        SHLog(@"subscribe temp publisher:%@ error: %@  failed publishers: %@",request.publishers,error,failedPublishers);
    }];
}
...
- (void)unsubscribeTempUserOnlineState:(NSString *)userId {
    if (![_subscribeIds containsObject:userId]) {
        ...
        [[NIMSDK sharedSDK].subscribeManager unSubscribeEvent:request completion:^(NSError * _Nullable error, NSArray * _Nullable failedPublishers) {
            SHLog(@"unSubscribe temp publisher:%@ error: %@  failed publishers: %@",request.publishers,error,failedPublishers);
        }];
...
- (void)publishOnlineState
{
    ...
    event.value = ESCustomStateValueOnlineExt;
    event.sendToOnlineUsersOnly = NO;  //必须要让后登录的用户也能拿到
    NSDictionary *ext = @{
//                          ESSubscribeNetState : @([ESDevice currentDevice].currentNetworkType),
                          ESSubscribeOnlineState : @(ESOnlineStateNormal), //移动端永远在线
                          };
    [event setExt:[self jsonString:ext]];
    [[NIMSDK sharedSDK].subscribeManager publishEvent:event completion:^(NSError * _Nullable error, NIMSubscribeEvent * _Nullable event) {
        SHLog(@"publish online state error %@ ext %@ time %.2f",error,ext,event.timestamp);
    }];
}
...
- (void)subscribeNextMembers:(NSArray *)ids
{
    ...
    [[NIMSDK sharedSDK].subscribeManager subscribeEvent:request completion:^(NSError * _Nullable error, NSArray * _Nullable failedPublishers) {
        SHLog(@"subscribe publisher:%@ error: %@  failed publishers: %@",request.publishers,error,failedPublishers);
...
- (void)onFriendChanged:(NIMUser *)user
{
    ...
    if (isMyFriend && ![self.subscribeIds containsObject:user.userId])
    {
        ...
        [[NIMSDK sharedSDK].subscribeManager subscribeEvent:request completion:^(NSError * _Nullable error, NSArray * _Nullable failedPublishers) {
            ...
            SHLog(@"subscribe publisher: %@ error: %@",request.publishers,error);
        }];
    }
...
- (void)onRecvSubscribeEvents:(NSArray *)events
{
    ...
    for (NIMSubscribeEvent *event in events) {
        if ([self.subscribeIds containsObject:event.from] || [self.tempSubscribeIds containsObject:event.from])
        {
            ...
            if (oldEvent.timestamp > event.timestamp)
            {
                ...
                SHLog(@"event id %@ from %@ is out of date, ingore...",event.eventId,event.from);
            }
            else
            {
                [eventsDict setObject:event forKey:event.from];
                SHLog(@"receive event id %@ from %@ time %.2f",event.eventId,event.from,event.timestamp);
            }
    ...
    if (unSubscribeUsers.count)
    {
        ...
        [[NIMSDK sharedSDK].subscribeManager unSubscribeEvent:request completion:^(NSError * _Nullable error, NSArray * _Nullable failedPublishers) {
            SHLog(@"unSubscribe publisher:%@ error: %@  failed publishers: %@",request.publishers,error,failedPublishers);
        }];
    }
}
