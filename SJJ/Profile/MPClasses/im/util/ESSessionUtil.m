...
static NSString *const ESRecentSessionAtMark = @"ESRecentSessionAtMark";

@implementation ESSessionUtil
...
+ (NSString*)showTime:(NSTimeInterval) msglastTime showDetail:(BOOL)showDetail {
    ...
    result = [ESSessionUtil getPeriodOfTime:hour withMinute:msgDateComponents.minute];
    if (hour > 12) {
        hour = hour - 12;
    }
    if(nowDateComponents.day == msgDateComponents.day) {//同一天,显示时间
        ...
        NSString *weekDay = [ESSessionUtil weekdayStr:msgDateComponents.weekday];
...
+ (NSDictionary *)dictByJsonString:(NSString *)jsonString {
    ...
    return [ESSessionUtil dictByJsonData:data];
}
...
+ (void)addRecentSessionAtMark:(NIMSession *)session {
    ...
    if (recent) {
        ...
        [dict setObject:@(YES) forKey:ESRecentSessionAtMark];
...
+ (void)removeRecentSessionAtMark:(NIMSession *)session {
    ...
    if (recent) {
        ...
        [localExt removeObjectForKey:ESRecentSessionAtMark];
...
+ (BOOL)recentSessionIsAtMark:(NIMRecentSession *)recent {
    NSDictionary *localExt = recent.localExt;
    return [localExt[ESRecentSessionAtMark] boolValue] == YES;
}
...
+ (NSString *)onlineState:(NSString *)userId detail:(BOOL)detail {
    NSString *state = @"";
    if (![ESSubscribeManager sharedManager] || [[NIMSDK sharedSDK].loginManager.currentAccount isEqualToString:userId])
    {
    ...
    NSDictionary *dict = [[ESSubscribeManager sharedManager] eventsForType:NIMSubscribeSystemEventTypeOnline];
    ...
    if ([info isKindOfClass:[NIMSubscribeOnlineInfo class]] && info.senderClientTypes.count) {
        ...
        switch (event.value) {
            case ESCustomStateValueOnlineExt:
...
+ (NSString *)resolveOnlineState:(NSString *)ext client:(NIMLoginClientType)client detail:(BOOL)detail
{
    ...
    NSDictionary *dict = [self jsonDict:ext];
    if (dict) {
        NSInteger stateValue = 0;
        id object = [dict objectForKey:ESSubscribeOnlineState];
        if ([object isKindOfClass:[NSString class]] || [object isKindOfClass:[NSNumber class]]) {
            stateValue = [object integerValue];
        }
//        NSString *netState =  [[ESDevice currentDevice] networkStatus:stateValue];
        
        ESOnlineState onlineState = stateValue;
        switch (onlineState) {
            case ESOnlineStateNormal:
            {
...
//                    }
                    ...
                    {
                        return [NSString stringWithFormat:@"在线"];
                    }
                }
            }
            case ESOnlineStateBusy:
                return @"忙碌";
            case ESOnlineStateLeave:
                return @"离开";
            default:
                break;
        }
...
static char ESStringJsonDictionaryAddress;

+ (NSDictionary *)jsonDict:(NSString *)string {
    NSDictionary *dict = [objc_getAssociatedObject(string, &ESStringJsonDictionaryAddress) copy];
    if (dict == nil) {   //解析过一次后就缓存解析结果，避免多次解析
        ...
        objc_setAssociatedObject(string,&ESStringJsonDictionaryAddress,dict,OBJC_ASSOCIATION_COPY);
    }
    return dict;
