...
- (instancetype)initWithAnchor:(ARPlaneAnchor *)anchor isHidden:(BOOL)hidden {
    ...
    SCNMaterial *lightMaterial = [SCNMaterial new];
    lightMaterial.diffuse.contents = [UIImage imageNamed:[NSString stringWithFormat:@"./art.scnassets/Materials/plane.png"]];
    lightMaterial.diffuse.wrapS = SCNWrapModeRepeat;
    lightMaterial.diffuse.wrapT = SCNWrapModeRepeat;
//    lightMaterial.diffuse.contents = [UIColor colorWithWhite:0.5 alpha:0.5];
    ...
    if (hidden) {
        ...
        self.planeGeometry.materials = @[transparentMaterial, transparentMaterial, transparentMaterial, transparentMaterial, lightMaterial, transparentMaterial];
    }
