...
#import <ESFoundation/ESFile.h>

@implementation ESFittingSampleRoomModel
...
- (void)updateImages
{
    ...
    {
        self.defaultImg = [ESFile getImageUrl:self.defaultImg withType:ESFileTypeHD];
    }
    else
    {
        self.defaultImg = @"";
...
- (void)updateLeftLabel:(NSArray *)labels
{
        ...
        && [labels[2] isKindOfClass:[NSString class]])
    {
        ...
        CGSize labelSize = [ESFittingSampleRoomModel getTextSize:label];
...
+ (CGSize)getTextSize:(NSString *)text
{
    ...
    // font需跟ESCaseFittingModelTagLabel中使用的font一致
