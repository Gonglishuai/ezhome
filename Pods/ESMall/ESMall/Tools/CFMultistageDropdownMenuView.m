...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        UILabel *line = [[UILabel alloc] initWithFrame:CGRectMake(0, frame.size.height-0.5, SCREEN_WIDTH, 0.5)];
...
{
    if (!_titleBar) {
        _titleBar = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, titleBarHeight)];
...
{
    if (!_backgroundView) {
        _backgroundView = [[UIView alloc] initWithFrame:CGRectMake(0, self.startY?:titleBarHeight, SCREEN_WIDTH, SCREEN_HEIGHT)];
...
- (CFMultistageConditionTableView *)multistageConditionTableView
{
    if (!_multistageConditionTableView) {
        _multistageConditionTableView = [[CFMultistageConditionTableView alloc] initWithFrame:CGRectMake(0, self.startY?:titleBarHeight, SCREEN_WIDTH, 0)];
...
- (void)setupTitleBarWithDefaulTitleArray:(NSArray *)defaulTitleArray
{
    ...
    CGFloat btnW = SCREEN_WIDTH/defaulTitleArray.count;  // -6 留下左边间距
    CGFloat btnH = 45;
    ...
    for (NSInteger i=0; i<defaulTitleArray.count; i++) {
        ...
        CFLabelOnLeftButton *titleBtn = [CFLabelOnLeftButton createButtonWithImageName:@"" title:@"" titleColor:[UIColor stec_titleTextColor] frame:CGRectMake(i*btnW+3, 0, btnW-6, btnH) target:self action:@selector(titleButtonClicked:)];
...
//            NSString *str = [NSString stringWithFormat:@"%@.png", self.stateConfigDict[@"normal"][1]];
        ...
        // 默认的
        btnImageName = @"";//图片名字
        btnTitleColor = [UIColor stec_titleTextColor];
    }
...
//        // 使用CFMultistageDropdownMenuView.bundle自带的
//        btnImageName = @"";//图片名字
//        btnTitleColor = [UIColor stec_blueTextColor];
//    }
//
