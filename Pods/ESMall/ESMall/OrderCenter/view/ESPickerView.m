...
@property(nonatomic,strong)UIControl *backHud;
...
- (void)addBackHud{
    _backHud = [[UIControl alloc]initWithFrame:(CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT))];
    [_backHud addTarget:self action:@selector(remove) forControlEvents:(UIControlEventTouchUpInside)];
}


-(NSArray *)getPlistArrayByplistName:(NSString *)plistName{
    
    NSString *path= [[ESMallAssets hostBundle] pathForResource:plistName ofType:@"plist"];
...
-(void)setFrame{
    ...
    CGFloat toolViewY= SCREEN_HEIGHT-toolViewH;
...
-(void)setUpDatePickerWithdatePickerMode:(UIDatePickerMode)datePickerMode{
    ...
    datePicker.backgroundColor=[UIColor colorWithRed:0.82f green:0.84f blue:0.87f alpha:1.00f];//colorWithRed:0.82f green:0.84f blue:0.87f alpha:1.00f];//[UIColor lightGrayColor];
    ...
    if (_minDate) {
        ...
        //        datePicker.maximumDate = [_minDate dateByAddingTimeInterval:60*60*24*365*2];
        currentYear=[[formatter stringFromDate:_minDate] integerValue];
        [datePicker setDate:_minDate];
        
    } else {
        if (_defaulDate) {
            datePicker.minimumDate = _defaulDate;
            //            datePicker.maximumDate = [_defaulDate dateByAddingTimeInterval:60*60*24*365*2];
            [datePicker setDate:_defaulDate];
            currentYear=[[formatter stringFromDate:_defaulDate] integerValue];
        } else {
            NSDate *date = [NSDate date];
            datePicker.minimumDate = date;
            //            datePicker.maximumDate = [date dateByAddingTimeInterval:60*60*24*365*2];
...
-(void)remove{
    ...
    if ([self.delegate respondsToSelector:@selector(toobarCancelBtnHaveClick:resultString:)]) {
        [self.delegate toobarCancelBtnHaveClick:self resultString:_resultString];
    }
    if (_backHud) {
        [_backHud removeFromSuperview];
    }
...
-(void)doneClick
{
    if (_pickerView) {
        ...
        if (_datePickerMode == UIDatePickerModeDateAndTime) {
            [fMinDate setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
        } else {
            [fMinDate setDateFormat:@"yyyy-MM-dd"];
        }
        _resultString=[NSString stringWithFormat:@"%@",[fMinDate stringFromDate:date]];
    }
