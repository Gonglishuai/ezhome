...
@interface ESPhotoBrowser () <ESPhotoViewDelegate>
{
    // 滚动的view
...
- (void)createToolbar
{
    ...
    CGFloat barY = self.view.frame.size.height - barHeight - 38;
    _toolbar = [[ESPhotoToolbar alloc] init];
...
- (void)showPhotos
{
	...
	for (ESPhotoView *photoView in _visiblePhotoViews)
    {
...
{
    ESPhotoView *photoView = [self dequeueReusablePhotoView];
    if (!photoView)
    { // 添加新的图片view
        photoView = [[ESPhotoView alloc] init];
...
- (ESPhotoView *)dequeueReusablePhotoView
{
    ESPhotoView *photoView = [_reusablePhotoViews anyObject];
	if (photoView) {
...
- (void)updateTollbarState
{
    ...
    if (self.delegate
        && [self.delegate respondsToSelector:@selector(photoBrowserDidChangedToPageAtIndex:)])
    {
        [self.delegate photoBrowserDidChangedToPageAtIndex:_currentPhotoIndex];
    }
}
